...
ASSET_PATHS = [
	ROOT / "assets" / "ship_hull.png",
	ROOT / "assets" / "ship_wing.png",
	ROOT / "assets" / "ship_exhaust.png",
	ROOT / "assets" / "enemy_1.png",
	ROOT / "assets" / "enemy_2.png",
	ROOT / "assets" / "enemy_3.png",
]
OUTPUT_DIR = ROOT / "tests" / "generated_transparent_assets"
PREVIEW_PATH = OUTPUT_DIR / "preview_strip.png"
...
def main() -> None:
	...
	for path in ASSET_PATHS:
		...
		preview_items.append((path.stem, cleaned))
		print(f"wrote {out_path.relative_to(ROOT)}")
	if preview_items:
		build_preview(preview_items).save(PREVIEW_PATH)
...
def clean_asset(path: Path) -> Image.Image:
	...
	background_mask = contract_bright_edge(pixels, background_mask, background_colors, passes=14)
	output = image.copy()
...
def edge_connected_background_mask(
	...
	width: int,
	height: int,
	background_colors: Iterable[tuple[int, int, int]],
) -> list[list[bool]]:
...
def remove_bright_outer_edge(image: Image.Image, passes: int = 1) -> Image.Image:
	output = image.copy()
	for _ in range(passes):
		pixels = output.load()
...
def fringe_alpha(rgb: tuple[int, int, int], background_colors: Iterable[tuple[int, int, int]]) -> int:
...
def pixel_matches_background(rgb: tuple[int, int, int], background_colors: Iterable[tuple[int, int, int]]) -> bool:
...
def nearest_color_distance(rgb: tuple[int, int, int], colors: Iterable[tuple[int, int, int]]) -> int:
...
def build_preview(items: list[tuple[str, Image.Image]]) -> Image.Image:
	card_width = 320
	card_height = 380
	preview = Image.new("RGBA", (card_width * len(items), card_height), (10, 18, 14, 255))
	for index, (_name, source) in enumerate(items):
		card = Image.new("RGBA", (card_width, card_height), (10, 18, 14, 255))
		checker = make_checkerboard(card_width - 32, card_height - 80)
		card.alpha_composite(checker, (16, 56))
		render = source.copy()
		render.thumbnail((card_width - 48, card_height - 96))
		x = (card_width - render.width) // 2
		y = 72 + (card_height - 96 - render.height) // 2
		card.alpha_composite(render, (x, y))
		preview.alpha_composite(card, (index * card_width, 0))
	return preview


def make_checkerboard(width: int, height: int, cell: int = 24) -> Image.Image:
	image = Image.new("RGBA", (width, height), (0, 0, 0, 0))
	pixels = image.load()
	for y in range(height):
		for x in range(width):
			even = ((x // cell) + (y // cell)) % 2 == 0
			value = 232 if even else 196

...
func _draw() -> void:
	...
	for i in range(_pulses.size()):
		...
		if i < _vis_pulse_polys.size() and _vis_pulse_polys[i] is PackedVector2Array and (_vis_pulse_polys[i] as PackedVector2Array).size() >= 2:
			draw_polyline(_vis_pulse_polys[i] as PackedVector2Array, arc_col, 1.2)
		else:
